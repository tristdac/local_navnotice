...
// In your Moodle page PHP file or within the relevant context
$PAGE->requires->js(new moodle_url($CFG->wwwroot . '/local/navnotice/js/check_contrast.js'));
...
class manage_navnotice_form extends moodleform {
    protected function definition() {
        ...
        // Separate inputs for background and text colours
        $mform->addElement('text', 'backgroundcolor', get_string('backgroundcolor', 'local_navnotice'));
        $mform->setType('backgroundcolor', PARAM_RAW);
        $mform->setDefault('textcolor', '#FFFFFF');
        $mform->hideIf('backgroundcolor', 'type', 'neq', 'navitem');

        $mform->addElement('text', 'textcolor', get_string('textcolor', 'local_navnotice'));
        $mform->setType('textcolor', PARAM_RAW);
        $mform->setDefault('textcolor', '#000000');
        $mform->hideIf('textcolor', 'type', 'neq', 'navitem');
    ...
    private function update_visibility($type) {
        ...
        if ($type == 'navitem') {
            ...
            $mform->getElement('backgroundcolor')->setAttributes(['style' => 'display:flex;']);
            $mform->getElement('textcolor')->setAttributes(['style' => 'display:flex;']);
            ...
            $mform->getElement('backgroundcolor')->setAttributes(['style' => 'display:none;']);
            $mform->getElement('textcolor')->setAttributes(['style' => 'display:none;']);
